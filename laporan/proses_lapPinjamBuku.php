...
while ($row = mysqli_fetch_array($query)){
  ...
  $pdf->Cell(1.5,0.5,$row['jml_pinjam'],1,1,'C'); 
   ...
   $query = mysqli_query($koneksi,"SELECT sum(jml_pinjam) as total FROM detil_pinjam,tb_peminjaman,tb_copybuku WHERE tb_copybuku.no_copy = detil_pinjam.no_copy AND tb_peminjaman.no_peminjaman = detil_pinjam.no_peminjaman AND status = '1' AND detil_pinjam.no_peminjaman is not null AND tb_peminjaman.tgl_pinjam BETWEEN '$Dari' AND '$Sampai' ORDER BY tb_peminjaman.tgl_pinjam asc");
